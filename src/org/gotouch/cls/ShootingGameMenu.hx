...
class ShootingGameMenu extends Sprite
{
	...
	private var Tickets:Array<Ticket2>;
	...
	{
		//Main.TopShootingLevel = 3;
		Tickets = new Array<Ticket2>();
		var Open:Bool = true;
		if (Main.TopShootingLevel == 0) {
			Open = false;
		}
		
		var Locked:Bool = false;
		for (i in 1...7) {	
			var Tiquete:Ticket2 = new Ticket2(i, Locked, Open);
			Tickets.push(Tiquete);
			if (i == Main.TopShootingLevel) {
				Open = false;
			}
			if (i == (Main.TopShootingLevel + 1)) {
				Locked = true;
			}
	...
	{
		var Target:Ticket2 = event.currentTarget;
		if(!Target.is_Locked){
			end();
			while (Lib.current.numChildren == 0) {
				Lib.current.addChild(new ShootingGame(Target.level));
			}
		}
	}
