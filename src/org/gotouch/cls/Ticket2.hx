...
class Ticket2 extends Sprite
{
	...
	public function new(number:Int,lock:Bool,finish:Bool) 
	{
		...
		if (finish) {
			Tiquete = Utils.loadGraphic(ConstantsAssets.SHOOTING_GAME_TICKET2, true);
		}
		if (!finish) {
			Tiquete = Utils.loadGraphic(ConstantsAssets.SHOOTING_GAME_TICKET1, true);			
		}
		Number  = Utils.loadGraphic(ConstantsAssets.SHOOTING_GAME_NUMBER + number + ".png", true);
